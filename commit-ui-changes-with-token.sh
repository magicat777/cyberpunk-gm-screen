...
# This script commits and pushes the UI modernization changes to GitHub using a token
# Run this script from your WSL home directory
# Usage: bash commit-ui-changes-with-token.sh YOUR_GITHUB_TOKEN

if [ -z "$1" ]; then
  echo "Please provide your GitHub personal access token"
  echo "Usage: bash commit-ui-changes-with-token.sh YOUR_GITHUB_TOKEN"
  exit 1
fi
...
if [ ! -d "$DEPLOY_DIR" ]; then
  ...
  # If directory exists, update the remote with token
...
cp "$MAIN_REPO/GITHUB-PAGES-REVIEW.md" . 2>/dev/null
cp "$MAIN_REPO/UI-MODERNIZATION-PLAN.md" . 2>/dev/null
cp "$MAIN_REPO/UI-MODERNIZATION-TASKS.md" . 2>/dev/null
